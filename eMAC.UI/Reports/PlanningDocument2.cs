        ...
        public PlanningDocument2(int meetingID)
        {
            ...
            if (_data.Tables.Count > 0)
            {
                ...
                if (_meeting_details.Rows.Count > 0)
                {
                    ...
                    budgetary_comments.Value = _meeting_details.Rows[0]["budgetary_comments"].ToString();

                    if(string.IsNullOrEmpty(_meeting_details.Rows[0]["budgetary_comments"].ToString()))
                    {
                        budgetary_comments.Visible = false;
                    }
                ...
                if (_meeting_budget.Rows.Count > 1)
                {
                    budget_info.Visible = false;
                    multiple_funding.DataSource = _meeting_budget;
                }
                else if (_meeting_budget.Rows.Count == 1)
                {
                    Dictionary<string, object> budget = _meeting_budget.ToDictionary().SingleOrDefault();
                    multiple_funding.Visible = false;
                    budget_info_estimated_cost.Value = string.Format("{0:###,###,###,###}", budget["estimated_cost"]);
                    budget_info_funds_available.Value = string.Format("{0:###,###,###,###}", budget["funds_available"]);
                    budget_info_ptaeo.Value = string.Format("{0}", budget["ptaeo"]);
                    budget_info_source_of_funds.Value = string.Format("{0}", budget["src_of_fund"]);
                }
