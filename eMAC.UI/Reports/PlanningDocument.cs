    ...
    public partial class PlanningDocument : Telerik.Reporting.Report
    {
        ...
        public PlanningDocument(int meetingID)
        {
            ...
            if (_data.Tables.Count > 0)
            {
                ...
                Dictionary<string, Dictionary<string, List<string>>> newCategory = new Dictionary<string, Dictionary<string, List<string>>>();

                var _categories = _pb.ToDictionary().Select(x => new { category = x["pb_category"].ToString() }).Distinct().ToList();
                var _outcomes = _pb.ToDictionary().Select(x => new { category = x["pb_category"].ToString(), outcome = x["pb_outcome"].ToString() }).Distinct().ToList();
                var _outputs = _pb.ToDictionary().Select(x => new { outcome = x["pb_outcome"].ToString(), output = x["pb_output"].ToString() }).Distinct().ToList();

                foreach (var i in _categories)
                {
                    if (newCategory.Where(x => x.Key == i.category).Count() == 0)
                    {
                        Dictionary<string, List<string>> newOutcome = new Dictionary<string, List<string>>();
                        foreach (var j in _outcomes.Where(x => x.category == i.category).ToList())
                        {
                            if (newOutcome.Where(x => x.Key == j.outcome).Count() == 0)
                            {
                                List<string> newOutput = new List<string>();
                                foreach (var k in _outputs.Where(x => x.outcome == j.outcome).ToList())
                                {
                                    newOutput.Add(k.output);
                                }
                                newOutcome.Add(j.outcome, newOutput);
                            }
                        }
                        newCategory.Add(i.category, newOutcome);
                    }
                }
                ...
                //            {
                //                var filteredOutput = _pb.Select(string.Format("pb_category='{0}' and pb_outcome='{1}'", dr["pb_category"].ToString(), fDR["pb_outcome"].ToString())).Distinct();
                ...
                //                    }
                //                }
                //                if (newOutcome.Keys.Where(x => x == dr["pb_outcome"].ToString()).Count() == 0)
                //                {
                //                    newOutcome.Add(dr["pb_outcome"].ToString(), newOutput);
                //                }
                //                else
                //                {
                //                    newOutcome.Add(dr["pb_outcome"].ToString() + outcomeCounter, newOutput);
                //                    outcomeCounter++;
                //                }
                //            }
                //        }
